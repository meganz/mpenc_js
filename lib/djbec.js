...
/**
 * Converts a byte array to a binary string (array of Uint8, retains endianness).
 */
...
/**
 * Converts a binary string to a byte array (array of Uint8, retains endianness).
 */
 ...
 */
djbec._hex2bytes = function(s) {
    var CHARS = '0123456789abcdef';
    var result = [];
    var i = 0;
    while (i < s.length) {
        result.push((CHARS.indexOf(s.charAt(i++)) << 4) + CHARS.indexOf(s.charAt(i++)));
    }
    return result;
};
 ...
 */
djbec._bytes2hex = function(s) {
    var CHARS = '0123456789abcdef';
    var result = '';
    for (var i = 0; i < s.length; i++) {
        result += CHARS.charAt(s[i] >> 4) + CHARS.charAt(s[i] & 15);
    }
    return result;
};
